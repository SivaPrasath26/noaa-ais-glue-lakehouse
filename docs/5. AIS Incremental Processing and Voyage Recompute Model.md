...
trajectory_points is partitioned by date + mmsi for window-scoped overwrite and pruning; rows are thinned (keep endpoints, moving fast/slow cadence by SOG, sparse anchored points). voyage_summary is small and written coalesced without partitions.
